...
- 管理后台专属功能
- 通过自然语言查询系统数据
- 支持用户/活跃用户/用量/知识库/智能体/API Key/中转配置/调用记录/生成内容统计
- 活跃用户统计（昨日/今日日活、本周/本月活跃用户数及活跃率）
- **每日明细数据**：返回本周/本月每日新增数量
- **趋势图表渲染**：AI自动生成ECharts配置，前端渲染可交互图表
- **图表类型切换**：柱状图/折线图/饼图切换，表格/图表切换
- 参数不明确时返回全部数据，由AI自行分析回答
- 支持启用/禁用 MCP 工具
- SSE 流式输出，多轮上下文对话
- 可配置上下文轮数（0-50轮）
- 自动切换文字/图片模型
...
- **系统问答** - 管理员通过自然语言查询系统数据，Function Calling 自动调用工具，每日明细趋势图表渲染
